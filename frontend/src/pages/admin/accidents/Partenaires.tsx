...
// Phase 1 : un seul produit dans cette sous-branche (RelaxAccidents Frais
// Médicaux) — RelaxAccidents Moto/Voyage rejoindront ce sélecteur plus tard.
const PRODUIT = "relaxaccidents_fraismedicaux";
const LIBELLE = "RelaxAccidents Frais Médicaux";
...
export default function AssurancesAccidentsPartenaires() {
  const [q, setQ] = useState("");
  const params = new URLSearchParams({ produit: PRODUIT });
  if (q) params.set("q", q);
  ...
  async function showQr(p: Partenaire) {
    try {
      const r = await api.get<{ dataUrl: string }>(`/partenaires/${p.id}/qr/${PRODUIT}`);
      setQr({ url: r.dataUrl, label: LIBELLE });
    } catch (err) {
      notify((err as Error).message);
    }
  }
  ...
  return (
    <>
      <PageHeader
        title="Partenaires — Assurances Accidents"
        subtitle="Réseau de distributeurs de RelaxAccidents Frais Médicaux et leurs QR codes."
      />
                ...
                <tbody>
                  {data.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td>{p.localisation ?? <span className="muted">—</span>}</td>
                      ...
                      <td>
                        <div style={{ display: "flex", gap: 6 }}>
                          <button className="btn btn-ghost" style={{ padding: 8 }} title={`QR ${LIBELLE}`} onClick={() => showQr(p)}>
                  ...
                  {data.length === 0 && (
                    <tr><td colSpan={5}><div className="empty">Aucun partenaire pour l'instant.</div></td></tr>
                  )}
                </tbody>
              </table>
            </div>
