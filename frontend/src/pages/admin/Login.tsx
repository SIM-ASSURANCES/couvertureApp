...
export default function AdminLogin() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setErreur("");
    try {
      await login(email, motDePasse, "/auth/admin/login");
      navigate("/admin");
    } catch (err) {
    ...
    >
      <div style={{ width: "100%", maxWidth: 420, background: "#fff", borderRadius: 20, overflow: "hidden", boxShadow: "0 24px 64px rgba(0,0,0,0.25)" }}>
        <div style={{ background: "linear-gradient(135deg, #004b9c 0%, #16215e 100%)", padding: "28px 32px 24px", color: "#fff" }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 800 }}>Administration</div>
          <div style={{ fontSize: 13, opacity: 0.8, marginTop: 4 }}>Connexion réservée à l'équipe SIM Assurances</div>
        </div>
